...
def record_radar(device_name, i, data_queue1, counter_flag):
	#global xep_rad1
	...
	for icounter_rad1 in range(nframes):
		...
		if len(dis_max_rad1) >= 3:
			...
			#print('Run task %s (%s)...' % (device_name, os.getpid()))
			print('radar_' + i + ': ', real_dis)
			#data_queue1.value = real_dis
...
def get_point(q1, q2, flag_r1, flag_r2):
	x_list = list()
	y_list = list()
	plt.ion()
	plt.figure(1)
	while True:
		...
		plt.pause(0.0001)
		if flag_r1.value == nframes - 1 and flag_r2.value == nframes - 1:
			plt.ioff()
			plt.show()
			#winsound.Beep(5000, 1000)
			break
...
if __name__ == "__main__":
	radar_q1 = Queue()
	radar_q2 = Queue()
	#radar_q1 = multiprocessing.Value("f", 0)
	#radar_q2 = multiprocessing.Value("f", 0)
	flag1 = multiprocessing.Value("i", 0)
	flag2 = multiprocessing.Value("i", 0)
	pg_rad1 = Process(target=record_radar, args=('COM3', '1', radar_q1, flag1))
	pg_rad2 = Process(target=record_radar, args=('COM4', '2', radar_q2, flag2))
